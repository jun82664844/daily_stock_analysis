...
- 本仓库已获得维护者对“当前任务范围内、已经验收通过的本地改动”的持续 `git commit` 授权；满足验证门禁、明确提交范围并显式暂存文件后，可直接完成本地提交，无需逐次询问。
- `git tag`、`git push`、创建或更新远端 PR / Release、生产环境操作、真实支付或密钥操作，以及 reset / clean / 强制覆盖等破坏性 Git 操作，仍必须逐次获得明确确认。
- 工作树出现其他窗口或未知来源的并发改动时，不得借持续授权顺带提交；应先盘点归属、隔离范围并完成对应验收。
