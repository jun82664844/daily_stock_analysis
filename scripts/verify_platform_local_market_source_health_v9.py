...
from scripts.verify_platform_local_query_resilience_v8 import run_local_query_resilience_v8_checks  # noqa: E402
from scripts.verify_platform_local_usability_v6 import MARKET_SNAPSHOT_EXPECTATIONS  # noqa: E402


OK_MARKER = "DSA_PLATFORM_LOCAL_MARKET_SOURCE_HEALTH_V9_OK"
...
class MarketSourceHealthV9Result:
    check_id: str
    title: str
    status: str
...
def _result(
    ...
    *,
    ...
    optional: bool = False,
) -> MarketSourceHealthV9Result:
    return MarketSourceHealthV9Result(
        check_id=check_id,
        title=title,
        status=status,
...
def _run_required_files_check(root: Path) -> MarketSourceHealthV9Result:
    started = time.monotonic()
    missing = sorted(rel_path for rel_path in REQUIRED_FILES if not (root / rel_path).exists())
    if missing:
        return _result(
            "v9_required_files_present",
            "Local Market Source Health V9 required files exist",
            started,
            status="failed",
...
def _run_verifiers_visible_check(root: Path) -> MarketSourceHealthV9Result:
    started = time.monotonic()
    if not _is_git_worktree(root):
        return _result(
            "v9_verifiers_visible_to_git",
            "V9 verifier files are not gitignored",
            started,
...
def _run_v9_unittest_check(root: Path, python_exe: str) -> MarketSourceHealthV9Result:
    started = time.monotonic()
    completed = subprocess.run(
        [python_exe, "-m", "unittest", "tests.test_platform_local_market_source_health_v9"],
        cwd=root,
        capture_output=True,
        text=True,
        encoding="utf-8",
    ...
    if completed.returncode != 0:
        return _result(
            "v9_backend_unittest",
            "V9 market-source health tests pass",
            started,
            status="failed",
            error="tests.test_platform_local_market_source_health_v9 failed",
            metadata=metadata,
        )
    return _result(
        "v9_backend_unittest",
...
def _run_v8_required_gate(
    *,
    ...
    if failed:
        return _result(
            "v8_query_resilience_gate",
            "V8 query resilience gate remains compatible with V9",
            started,
            status="failed",
            error="V8 verifier has failed checks",
            metadata=metadata,
        )
    return _result(
        "v8_query_resilience_gate",
...
def evaluate_snapshot_source_health(results: dict[str, dict[str, Any]]) -> list[str]:
    ...
    for symbol, item in sorted(results.items()):
        ...
        if not isinstance(source_health, dict):
            bad_symbols.append(symbol)
            continue
        quote = source_health.get("quote")
        history = source_health.get("history")
        if not isinstance(quote, dict) or not isinstance(history, dict):
            bad_symbols.append(symbol)
            continue
        if quote.get("status") not in valid_statuses or history.get("status") not in valid_statuses:
            bad_symbols.append(symbol)
            continue
        if not quote.get("source") or not history.get("source"):
            bad_symbols.append(symbol)
    return bad_symbols


def _run_optional_live_source_health_check(base_url: str) -> MarketSourceHealthV9Result:
    started = time.monotonic()
    email, password = _new_smoke_credentials("local-v9-source-health")
    ...
    try:
        ...
        if register_status >= 400:
            return _result(
                "optional_live_source_health",
                "Optional live V9 source-health smoke",
                started,
                status="degraded",
    ...
    bad_source_health = evaluate_snapshot_source_health(snapshots)
    ai_used = [symbol for symbol, item in snapshots.items() if item.get("ai_used") is not False]
    if bad_source_health or ai_used:
        return _result(
            "optional_live_source_health",
            "Optional live V9 source-health smoke",
            started,
            status="degraded",
            error="one or more live snapshots lacked source-health diagnostics or no-AI boundary",
            metadata={**metadata, "bad_source_health_symbols": bad_source_health, "ai_used_symbols": ai_used},
            optional=True,
        )
    return _result(
        "optional_live_source_health",
...
def run_local_market_source_health_v9_checks(
    *,
    ...
    run_live: bool = True,
) -> list[MarketSourceHealthV9Result]:
    ...
    if run_subprocess:
        results.append(_run_v9_unittest_check(root, py))
        results.append(
            _run_v8_required_gate(
                root=root,
                python_exe=py,
                base_url=base_url,
    ...
    if run_live:
        results.append(_run_optional_live_source_health_check(base_url))
    return results


def _passed_for_marker(results: Sequence[MarketSourceHealthV9Result]) -> bool:
    return all(result.status != "failed" for result in results)


def _print_text_report(results: Sequence[MarketSourceHealthV9Result]) -> None:
    for result in results:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Verify DSA platform Local Market Source Health V9 closure")
    ...
    parser.add_argument("--skip-subprocess", action="store_true", help="Skip V9 unittest and V8 verifier subprocess checks")
