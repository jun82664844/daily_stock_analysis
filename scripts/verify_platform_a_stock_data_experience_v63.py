from __future__ import annotations

import json
import time
from dataclasses import dataclass
from pathlib import Path


REPO_ROOT = Path(__file__).resolve().parents[1]
OK_MARKER = "DSA_PLATFORM_A_STOCK_DATA_EXPERIENCE_V63_OK"


@dataclass(frozen=True)
class CheckResult:
    check_id: str
    title: str
    status: str
    elapsed_sec: float
    missing: list[str]

    def to_dict(self) -> dict:
        return {
            "check_id": self.check_id,
            "title": self.title,
            "status": self.status,
            "elapsed_sec": round(self.elapsed_sec, 3),
            "missing": self.missing,
        }


REQUIRED_MARKERS: dict[str, list[str]] = {
    "src/services/a_share_enrichment_service.py": [
        '"reader_summary"',
        "def _reader_summary",
        '"今日关键信息"',
        '"miss_explanations"',
        '"premium_features"',
        "仅作信息分析，不构成投资建议。",
    ],
    "tests/test_a_share_enrichment_service.py": [
        "test_a_stock_data_payload_includes_v63_reader_summary",
        "reader_summary",
        "为什么值得看",
        "今日关键信息",
        "公告原文",
    ],
    "api/v1/schemas/basic_query.py": [
        "BasicAShareReaderSummaryPayload",
        "BasicAShareReaderFactPayload",
        "BasicAShareReaderMissPayload",
        "reader_summary",
    ],
    "tests/test_basic_query_no_ai.py": [
        "test_snapshot_accepts_a_share_source_mode_query_param",
        '"reader_summary"',
        "600519 A-share readout",
        "Refresh later.",
    ],
    "apps/dsa-web/src/api/stocks.ts": [
        "readerSummary",
        "keyFacts",
        "missExplanations",
        "premiumFeatures",
    ],
    "apps/dsa-web/src/pages/HomePage.tsx": [
        "basic-query-a-share-reader-summary",
        "未命中说明",
        "版本差异",
        "Useful readout",
    ],
    "apps/dsa-web/src/pages/__tests__/HomePage.test.tsx": [
        "renders the V63 A-share reader summary in Chinese mode",
        "basic-query-a-share-reader-summary",
        "今日关键信息",
        "版本差异",
    ],
    "scripts/verify_platform_a_stock_data_experience_v63.py": [
        OK_MARKER,
        "reader_summary",
        "basic-query-a-share-reader-summary",
    ],
    "tests/test_platform_a_stock_data_experience_v63.py": [
        "test_verifier_passes_when_all_v63_markers_exist",
        "test_verifier_fails_when_reader_summary_marker_is_missing",
        OK_MARKER,
    ],
    ".gitignore": [
        "!scripts/verify_platform_a_stock_data_experience_v63.py",
    ],
    "docs/superpowers/plans/2026-07-08-dsa-v63-a-stock-data-experience.md": [
        OK_MARKER,
        "local-only",
        "No AI calls",
        "No public search",
        "reader_summary",
        "not investment advice",
        "Do not commit real API Key",
    ],
    "docs/superpowers/platform-review-slices.md": [
        "V63 A-Stock-Data Experience Addendum",
        "scripts/verify_platform_a_stock_data_experience_v63.py",
        OK_MARKER,
    ],
    "docs/superpowers/platform-release-candidate-manifest.md": [
        "V63 A-Stock-Data Experience Manifest Addendum",
        "scripts/verify_platform_a_stock_data_experience_v63.py",
        OK_MARKER,
    ],
}


def _read_text(root: Path, rel_path: str) -> str:
    return (root / rel_path).read_text(encoding="utf-8", errors="replace")


def _check_markers(root: Path, rel_path: str, markers: list[str]) -> CheckResult:
    started = time.monotonic()
    path = root / rel_path
    if not path.exists():
        return CheckResult(rel_path, rel_path, "failed", time.monotonic() - started, ["file missing"])
    text = _read_text(root, rel_path)
    missing = [marker for marker in markers if marker not in text]
    return CheckResult(
        rel_path,
        rel_path,
        "failed" if missing else "passed",
        time.monotonic() - started,
        missing,
    )


def run_checks(root: Path = REPO_ROOT) -> list[CheckResult]:
    return [_check_markers(root, rel_path, markers) for rel_path, markers in REQUIRED_MARKERS.items()]


def main() -> int:
    results = run_checks(REPO_ROOT)
    failed = [result for result in results if result.status != "passed"]
    for result in results:
        prefix = "[OK]" if result.status == "passed" else "[FAIL]"
        print(f"{prefix} {result.check_id}")
        if result.missing:
            print(f"      missing={json.dumps(result.missing, ensure_ascii=False)}")
    print(json.dumps([result.to_dict() for result in results], ensure_ascii=False, indent=2))
    if failed:
        print("DSA_PLATFORM_A_STOCK_DATA_EXPERIENCE_V63_FAILED")
        return 1
    print(OK_MARKER)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
