from __future__ import annotations

import json
import time
from dataclasses import dataclass
from pathlib import Path


REPO_ROOT = Path(__file__).resolve().parents[1]
OK_MARKER = "DSA_PLATFORM_FREE_DETAIL_READABILITY_V70_OK"


@dataclass(frozen=True)
class CheckResult:
    check_id: str
    status: str
    elapsed_sec: float
    missing: list[str]

    def to_dict(self) -> dict:
        return {
            "check_id": self.check_id,
            "status": self.status,
            "elapsed_sec": round(self.elapsed_sec, 3),
            "missing": self.missing,
        }


REQUIRED_MARKERS: dict[str, list[str]] = {
    "apps/dsa-web/src/pages/HomePage.tsx": [
        "basic-query-data-detail-core",
        "basic-query-data-detail-events",
        "basic-query-peer-table",
        "basic-query-kline-triggers",
        "展开详情",
        "事件清单",
        "同业对比表",
        "K线触发条件",
        "Peer comparison table",
        "K-line triggers",
        "Technology sector ETF",
        "科技行业 ETF",
        "使用深度分析或配置资讯源",
    ],
    "apps/dsa-web/src/pages/__tests__/HomePage.test.tsx": [
        "basic-query-data-detail-core",
        "basic-query-data-detail-events",
        "basic-query-peer-table",
        "basic-query-kline-triggers",
        "展开详情",
        "事件清单",
        "同业对比表",
        "K线触发条件",
        "科技行业 ETF",
        "使用深度分析或配置资讯源",
    ],
    ".gitignore": [
        "!scripts/verify_platform_free_detail_readability_v70.py",
    ],
    "docs/superpowers/plans/2026-07-08-dsa-v70-free-detail-readability.md": [
        OK_MARKER,
        "local-only",
        "No AI calls",
        "No public search",
        "free detail readability",
        "expand details",
        "event checklist",
        "peer comparison table",
        "K-line triggers",
        "same visible modules",
        "not investment advice",
        "Do not commit real API Key",
    ],
    "docs/superpowers/platform-review-slices.md": [
        "V70 Free Detail Readability Addendum",
        OK_MARKER,
        "scripts/verify_platform_free_detail_readability_v70.py",
    ],
    "docs/superpowers/platform-release-candidate-manifest.md": [
        "V70 Free Detail Readability Manifest Addendum",
        OK_MARKER,
        "tests/test_platform_free_detail_readability_v70.py",
    ],
    "scripts/verify_platform_release_candidate_package.py": [
        "verify_platform_free_detail_readability_v70.py",
        "test_platform_free_detail_readability_v70.py",
        "2026-07-08-dsa-v70-free-detail-readability.md",
    ],
    "tests/test_platform_release_candidate_package.py": [
        "include_v70",
        "test_reports_missing_free_detail_readability_v70_files",
        "verify_platform_free_detail_readability_v70.py",
        "test_platform_free_detail_readability_v70.py",
        "2026-07-08-dsa-v70-free-detail-readability.md",
    ],
}


def _read_text(root: Path, rel_path: str) -> str:
    return (root / rel_path).read_text(encoding="utf-8", errors="replace")


def _check_markers(root: Path, rel_path: str, markers: list[str]) -> CheckResult:
    started = time.monotonic()
    path = root / rel_path
    if not path.exists():
        return CheckResult(rel_path, "failed", time.monotonic() - started, ["file missing"])
    text = _read_text(root, rel_path)
    missing = [marker for marker in markers if marker not in text]
    return CheckResult(rel_path, "failed" if missing else "passed", time.monotonic() - started, missing)


def run_checks(root: Path = REPO_ROOT) -> list[CheckResult]:
    return [_check_markers(root, rel_path, markers) for rel_path, markers in REQUIRED_MARKERS.items()]


def main() -> int:
    results = run_checks(REPO_ROOT)
    failed = [result for result in results if result.status != "passed"]
    for result in results:
        prefix = "[OK]" if result.status == "passed" else "[FAIL]"
        print(f"{prefix} {result.check_id}")
        if result.missing:
            print(f"      missing={json.dumps(result.missing, ensure_ascii=False)}")
    print(json.dumps([result.to_dict() for result in results], ensure_ascii=False, indent=2))
    if failed:
        print("DSA_PLATFORM_FREE_DETAIL_READABILITY_V70_FAILED")
        return 1
    print(OK_MARKER)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
