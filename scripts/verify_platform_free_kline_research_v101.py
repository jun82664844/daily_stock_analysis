...
OK_MARKER = "DSA_PLATFORM_FREE_KLINE_RESEARCH_V101_OK"
REQUIRED_FILES = (
...
def _required_files(root: Path) -> CheckResult:
    ...
    return CheckResult(
        check_id="v101_required_files",
        title="V101 required files exist",
...
def _git_visibility(root: Path) -> CheckResult:
    started = time.monotonic()
    verifier = "scripts/verify_platform_free_kline_research_v101.py"
    ...
    return CheckResult(
        check_id="v101_verifier_visible",
        title="V101 verifier is visible to git",
...
def run_v101_checks(*, project_root: Path = REPO_ROOT, run_subprocess: bool = True) -> list[CheckResult]:
    ...
    results.extend([
        _command(
            "v101_api_and_component_tests",
            "V101 API and research component tests pass",
            web,
            [npm, "run", "test", "--", "--run", "src/api/__tests__/stocks.test.ts", "src/components/research/__tests__/FreeKlineResearchV101.test.tsx"],
        ),
        _command(
            "v101_homepage_integration",
            "V101 guest quick-analysis integration passes",
            web,
            [npm, "run", "test", "--", "--run", "src/pages/__tests__/HomePage.test.tsx", "-t", "routes guest quick analysis to the free no-AI snapshot"],
        ),
        _command(
            "v101_frontend_build",
            "V101 frontend production build passes",
            web,
            [npm, "run", "build"],
        ),
    ])
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Verify the DSA V101 free K-line research panel.")
    parser.add_argument("--project-root", default=str(REPO_ROOT))
    parser.add_argument("--skip-subprocess", action="store_true")
    args = parser.parse_args(argv)
    results = run_v101_checks(
    ...
    if failed:
        print("DSA_PLATFORM_FREE_KLINE_RESEARCH_V101_FAILED")
        return 1
    print(OK_MARKER)
    return 0
