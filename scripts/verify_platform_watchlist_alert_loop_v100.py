...
OK_MARKER = "DSA_PLATFORM_WATCHLIST_ALERT_LOOP_V100_OK"
REQUIRED_FILES = (
...
def _required_files(root: Path) -> CheckResult:
    ...
    return CheckResult(
        check_id="v100_required_files",
        title="V100 required files exist",
...
def _git_visibility(root: Path) -> CheckResult:
    started = time.monotonic()
    verifier = "scripts/verify_platform_watchlist_alert_loop_v100.py"
    ...
    return CheckResult(
        check_id="v100_verifier_visible",
        title="V100 verifier is visible to git",
    ...
    *,
    ...
    results.extend(
        [
            _command(
                "v100_backend_tests",
                "V100 backend and verifier tests pass",
                root,
                [python_path, "-m", "unittest", "tests.test_platform_watchlist_alert_loop_v100", "tests.test_platform_watchlist_alert_loop_v100_verifier", "tests.test_platform_watchlist_event_radar_v99"],
            ),
            _command(
                "v100_frontend_components",
                "V99/V100 radar component tests pass",
                root / "apps" / "dsa-web",
                [npm, "run", "test", "--", "--run", "src/components/radar/__tests__/WatchlistEventRadarV99.test.tsx", "src/components/radar/__tests__/WatchlistAlertLoopV100.test.tsx"],
            ),
            _command(
                "v100_frontend_homepage",
                "V100 HomePage integration test passes",
                root / "apps" / "dsa-web",
                [npm, "run", "test", "--", "--run", "src/pages/__tests__/HomePage.test.tsx", "-t", "V100 saved review loop"],
            ),
            _command(
                "v100_frontend_build",
                "V100 frontend production build passes",
                root / "apps" / "dsa-web",
                [npm, "run", "build"],
            ),
        ]
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Verify the local DSA V100 private watchlist alert loop.")
    parser.add_argument("--project-root", default=str(REPO_ROOT))
    parser.add_argument("--python", default=sys.executable)
    parser.add_argument("--skip-subprocess", action="store_true")
    args = parser.parse_args(argv)
    results = run_v100_checks(
    ...
    if failed:
        print("DSA_PLATFORM_WATCHLIST_ALERT_LOOP_V100_FAILED")
        return 1
    print(OK_MARKER)
    return 0
