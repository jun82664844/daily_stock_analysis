...
- [研究] V132 为具有关联证券的公开市场事件新增“事件研究卡”，集中展示事件事实、来源数量、关联证券和最新公开行情背景。
- [数据] V132 复用首页已经加载的 A股、港股、美股活跃榜、涨跌榜、关注列表和指数数据，不新增网络请求、AI 调用或额度消耗。
- [透明] 研究卡展示最新价、涨跌幅、成交量、成交额、行情来源、新鲜度和观测时间；榜单缺失时明确降级，不伪造价格或走势。
- [合规] V132 明确说明事件与价格变化同时呈现不代表因果关系，只提供公开资讯和数据，不构成投资建议。
- [测试] V132 覆盖研究入口、行情可用、行情缺失、查询、关闭、中英文和独立发布门禁。
