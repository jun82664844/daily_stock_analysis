...
- [留存] V142 将快速查询结果重构为“个股研究总览”，游客和免费用户无需登录、API Key 或 AI，即可在同一阅读路径查看行情、财务趋势、同业、资讯事件、K线样本和来源边界。
- [数据] 新增按需加载的 A股、港股、美股公开年度财务接口，最多展示五个财年收入、净利润、摊薄每股收益、经营现金流及增长率；公开源缺失时保持不可用，不补零。
- [估值] 历史市盈率仅按财年末观察价格除以当年摊薄每股收益计算，展示样本区间和百分位，并明确不等同于实时、预测或目标估值。
- [体验] 快速查询默认收拢旧版重复长模块，通过“展开全部详细模块”保留完整明细；六个标签页和降级状态均支持中文、英文及移动端横向滚动。
- [性能] 财务数据在用户进入“财务趋势”标签后才加载，并使用六小时本地内存缓存；财务源失败不会阻断行情、同业、事件或K线阅读。
- [边界] V142 只提供资讯和数据，不生成投资建议、交易指令、目标价、收益预测、仓位或止损；不调用平台模型、BYOK、Ollama、Kronos 或公共搜索。
- [留存] V141 在个股事件档案内新增“同类事件历史对比”，游客和免费用户无需登录、API Key 或 AI，即可查看当前证券过去同类事件后的客观价格分布。
- [数据] 财报、分红、拆股、回购和重要公告按 1/3/5/20 个交易日分别统计样本数、个股中位数、区间、市场基准中位数、相对表现中位数及正负样本数量；无效、缺失和非有限值不计入统计，也不补零。
- [体验] 当前事件类型、观察窗口和历史样本选择与 V139 列表、V140 K 线图双向联动；样本少于 3 个或暂无观察时明确提示，不输出误导性的高置信度结论。
- [边界] V141 只展示同一证券的公开历史描述统计，不预测未来表现、不证明因果关系，不生成影响评级、目标价、收益预测、仓位或交易指令。
- [测试] V141 收口同步修正 V136 verifier 的旧模拟 endpoint，使测试夹具与现行 45 日历史窗口配置函数保持一致。
- [留存] V140 在 V139 个股事件档案内新增“事件与K线联动图”，游客和免费用户无需登录、API Key 或 AI，即可把公开事件、个股历史行情和市场基准放在同一时间轴查看。
- [数据] 图表支持 A 股、港股和美股，分别对照上证指数、恒生指数和标普500指数；按 6/12/24 个月返回最多 560 个日线点，以区间首个有效收盘归一化为 0，并保留原始收盘与成交量。
- [体验] 财报、分红、拆股等事件可在图上选择，当前 1/3/5/20 个交易日观察窗口同步高亮；基准缺失时保留个股曲线，历史行情缺失时保留事件档案并明确降级。
- [边界] V140 只展示公开历史同期数据，不运行 Kronos、平台模型、BYOK 或 Ollama，不生成影响评级、目标价、收益预测、仓位或交易指令；事件与价格同图不代表因果关系。
- [留存] V139 在个股市场工作区新增“个股事件档案”，游客和免费用户无需登录、API Key 或 AI，即可按 6/12/24 个月查看公开事件记录。
- [数据] 档案复用巨潮资讯、Yahoo 公开日历与公司行动记录，并展示事件后 1/3/5/20 个交易日的个股、市场基准和相对变化；缺失观察保持不可用。
- [体验] 中英文页面支持时间、事件类型和观察窗口筛选，保留来源链接、来源状态和数据新鲜度；档案失败不阻塞基础行情。
- [边界] 当前真实接入事件为定期披露、分红除息和拆股；回购及重要公告保留筛选契约但不伪造数据。同期变化不表示事件导致价格变化，只提供资讯和数据。
- [安全] V139 使用独立匿名 IP 限流，生产模板默认关闭，不调用 AI、不读取用户密钥、不写数据库、不发送通知，不提供投资建议。

- [覆盖] V138 修复指定 A 股被全市场事件截断的问题，并将巨潮实际披露日期标记为来源已记录的历史事件。
- [数据] V138 新增 Yahoo 公共图表历史公司行动通道，补充港股和美股除息、拆股日期；不把新闻时间或模型推测当作事件日期。
- [观察] 历史回看扩展到 180 天、行情读取扩展到一年；最多六条观察按 A股、港股、美股保留可用市场席位，再按日期填充。
- [透明] API 和中英文页面区分 `provider_schedule / scheduled` 与 `provider_event_history / observed`，来源明确显示巨潮资讯、Yahoo 公开日历和图表。
- [安全] V138 保持免 AI、无密钥、只读公开数据和生产默认关闭；同期表现不解释为事件因果，不生成投资建议、目标价、收益预测、仓位或交易指令。
- [测试] 新增指定标的过滤、A 股实际披露、港股公司行动、三市场均衡、一年行情、双语事件依据和独立 V138 门禁。

- [性能] V137 为公开事件后市场观察增加版本化磁盘快照；服务重启后可先返回最近一次有效公开快照，再进行单飞后台刷新。
- [可用性] A股、港股和美股事件候选改为独立任务与独立状态，单市场超时不再阻塞或清空其他市场。
- [稳定性] 未完成的日历任务按查询键复用，事件观察不再同步构建完整市场首页；三市场全部失败时保留上一份有效快照。
- [安全] 快照最大 1 MiB，采用原子替换，只允许公开响应字段，递归拒绝 API Key、令牌、Cookie、用户 ID、邮箱和违反免 AI/仅资讯边界的数据。
- [体验] 页面显示三市场来源、内存/磁盘缓存和后台刷新状态，无快照时快速返回刷新状态并进行最多六轮有限轮询。
- [配置] 本地可启用 FastAPI 启动预热；通用示例和生产模板默认关闭，不配置 Windows 自动启动。
- [合规] V137 只改善公开资讯和历史数据的可用性，不把事件同期表现解释为因果，不生成投资建议、目标价、收益预测、仓位或交易指令。
- [测试] 新增持久快照、损坏/过期/敏感字段、全来源失败保留旧快照、单飞刷新、三市场隔离、任务复用、启动预热、双语状态和独立门禁覆盖。

- [留存] V136 在真实市场日历下新增“事件后市场观察”，游客和免费用户无需登录、API Key 或 AI，即可查看已发生计划事件后 1/3/5/20 个交易日的客观行情。
- [数据] 分别展示证券同期变化、上证指数/恒生指数/标普500指数同期变化、相对基准变化和成交量比；数据不足时显示等待或不可计算，不补造数值。
- [透明] 仅处理来源明确的 `scheduled/provider_schedule` 历史事件；Yahoo Chart 只标记为无密钥公开行情通道，不声称交易所实时、官方授权或完整企业行动调整。
- [性能] 最多处理最近 6 个事件，证券和基准历史并发读取，整体等待上限 4 秒；响应缓存 15 分钟，来源失败可使用最多 6 小时陈旧缓存并明确标记。
- [口径] V136 从最近 45 日真实事件中取样，以事件所在市场的事件日收盘为基线，按市场基准交易日确定后续 1/3/5/20 日；个股停牌或缺少目标交易日时保持不可计算，不会跨过停牌日误取更晚行情。
- [覆盖] 历史日历会补取跨越 45 日窗口所需的上一巨潮报告期和上一年度 FOMC 日程；单来源缓存最多保留 72 条事件，避免大列表长期驻留。
- [降级] 事件源全部超时或“部分成功但实际无事件”时不会被缓存为正常空结果；历史行情陈旧降级保留原抓取时间，行情缓存最多保留 32 个代码，日历缓存最多保留 64 个键。部分来源失败仍显示成功事件并明确告警；若响应违反免 AI/仅资讯边界，前端拒绝展示整批数据。
- [安全] 新公开端点不解析用户身份，前端不上传用户标识或跨站凭据；即使同源浏览器附带已有会话 Cookie，端点也不读取其身份。全局平台限流关闭时仍执行独立 IP 限流，进程内限流桶最多保留 4096 个身份；容量已满时拒绝新身份，不淘汰仍在冷却期的活跃桶。生产配置默认关闭，不调用 AI、不读取平台或用户 API Key、不写数据库、不发送通知。
- [合规] 页面明确说明同期表现不代表事件导致行情变化，只提供资讯和数据，不生成因果判断、影响评级、目标价、收益预测、仓位或交易指令。
- [测试] V136 覆盖三市场代码与基准、观察窗口、量能比、未来/数据不足状态、并发超时、缓存降级、安全响应、双语交互、失败隔离和独立发布门禁。

- [留存] V135 将 V134 本地提醒升级为真实公开市场日历，游客和免费用户无需登录、API Key 或 AI 即可查看今日、本周、我的日历和待复盘。
- [数据] A股定期报告预约来自巨潮资讯公开接口；港美股财报与除息日期来自 Yahoo Finance 的 yfinance 无密钥公开通道；宏观日程来自美联储 FOMC 官方页面。
- [透明] 计划事件使用 `scheduled` 时间依据和 `provider_schedule` 分类来源，只显示来源明确提供的日期，不展示盈利预测、收入预测、目标价或收益预测。
- [性能] 三类来源并行加载，整体等待上限 4 秒；每个来源独立缓存 15 分钟并允许最多 6 小时陈旧缓存降级，单一来源失败不会清空其他市场或既有新闻。
- [安全] V135 首页最多保留 36 条真实日程，和新闻合并后最多 48 条；生产配置默认关闭，本地验收配置开启，不调用 AI、不读取用户 API Key、不写数据库。
- [浏览器] 平台认证启用时，游客不再预取历史、自选、设置状态和分析技能等私有资源；公开市场首页与免登录查询仍正常加载，避免无意义的 401 请求。
- [合规] 页面持续标注公开数据、未使用 AI、事件与行情并列不代表因果关系，以及“仅提供资讯和数据，不构成投资建议”。
- [测试] V135 覆盖三市场来源、预约变更、已披露过滤、缓存、陈旧降级、超时、窗口/容量、安全 URL、FOMC 解析、契约合并、中英文四视图和独立发布门禁。

- [留存] V134 在免费“今日市场事件”内新增市场日历，汇总 A 股、港股、美股公开事件和 V133 关注后的 1/3/5/20 日复盘节点。
- [透明] 只有标题或摘要明确给出 ISO、中文月日或英文月份日期时才标记“计划日期”；其他公开资讯显示为“发布时间”，不把新闻发布时间包装成未来安排。
- [个性化] 日历支持“全部日历 / 我的日历 / 待复盘”，自选代码采用精确市场匹配；关注或取消 V133 事件后，复盘节点实时同步。
- [可用性] 当日公开事件超过默认展示上限时，优先保留最多 4 个近期关注复盘节点，避免用户刚关注的检查点被高密度资讯流截断。
- [隐私] 待复盘查看状态只保存在浏览器本地，最多 128 条；游客登录后仅迁入当前数字用户 ID 作用域，不保存邮箱、不上传状态。
- [性能] V134 复用已经加载的公开事件和关注记录，只保留最近 7 日和未来 30 日、最多 48 条，不新增网络请求、AI、API 额度或数据库写入。
- [合规] 日历明确显示日期依据、来源状态和非因果说明；只整理公开资讯和本地检查点，不构成投资建议。
- [测试] V134 覆盖三种日期格式、非法日期、三市场、时间窗口、容量上限、精确代码匹配、游客迁移、中英文、待复盘确认、关注联动和独立发布门禁。

- [留存] V133 为公开市场事件新增浏览器本地“事件后续追踪”，游客可关注事件，登录后自动迁入当前数字用户 ID 的私有作用域。
- [数据] V133 记录关注时公开行情基准和后续访问时行情观察，并展示 1/3/5/20 日观察节点；同一 UTC 日期只保留一条更新记录。
- [资讯] V133 自动汇总同一证券、晚于原事件的后续公开事件，不做模糊代码匹配，不把市场指数误配为个股。
- [隐私] V133 最多保存 12 个事件、每项 32 条观察，不保存邮箱，不上传关注状态，不新增网络请求、AI 调用、API 额度或数据库写入。
- [合规] 事件和后续行情仅并列展示，不代表因果关系；只提供公开资讯和数据，不构成投资建议。
- [测试] V133 覆盖本地存储、游客迁移、用户隔离、同日覆盖、观察节点、后续事件、中英文、关注/取消和独立发布门禁。

- [新功能] V132 为具有关联证券的公开市场事件新增“事件研究卡”，集中展示事件事实、来源数量、关联证券和最新公开行情背景。
- [改进] V132 复用首页已经加载的 A股、港股、美股活跃榜、涨跌榜和关注列表，不新增网络请求、AI 调用或额度消耗。
- [修复] V132 的关联证券行情候选排除市场指数，避免同号指数与个股（如 `000001.SH` 与 `000001.SZ`）被错误关联。
- [改进] 研究卡展示最新价、涨跌幅、成交量、成交额、行情来源、新鲜度和观测时间；榜单缺失时明确降级，不伪造价格或走势。
- [改进] V132 明确说明事件与价格变化同时呈现不代表因果关系，只提供公开资讯和数据，不构成投资建议。
- [测试] V132 覆盖研究入口、完整与部分行情缺失、查询、关闭、焦点恢复、中英文和独立发布门禁。

- [留存] V131 为回访用户新增“只看新增 / New only”事件收件箱，摘要同时展示总事件、新增、重点和可选的自选相关数量。
- [透明] V131 新增模式仅按浏览器本地有限容量事件 ID 过滤；不会改写事件、来源或完整公共事件流，读完后可一键返回全部事件。
- [隐私] V131 不上传已读状态、自选、筛选偏好或用户标识，不新增网络请求、AI 调用、API 额度、数据库表或服务端用户追踪。
- [合规] V131 只提供公开资讯和数据，不生成目标价、收益预测、买卖建议、仓位或交易指令。
- [测试] V131 覆盖双语摘要、只看新增、市场筛选、已读状态不变、真实空状态、恢复完整事件流和独立发布门禁。
- [修复] V131 验收期间移除三个既有页面图标按钮的原生 `title`，改用可访问名称，避免浏览器原生提示与统一 Tooltip 规则冲突。
- [测试] 配对码倒计时测试改为等待 React effect 提交后的真实状态，避免在初始 `0:00` 渲染帧上产生稳定误报。

- [留存] V130 登录且有自选的用户在“今日市场事件”中默认看到“为我优先”，按自选股、相关行业、关注市场、普通事件四级排序，并可切回“全部事件”。
- [解释] V130 每条优先事件显示“自选相关 / 相关行业 / 关注市场”依据；个性化只改变顺序，不隐藏公共事件。
- [隐私] V130 自选代码、行业推导和排序全部在浏览器端完成，公共首页 API 不接收用户标识或自选数据；游客不显示个性化控件。
- [性能] V130 行业信息复用已经加载的公共市场快照，不新增网络请求、AI 调用或额度消耗；首页主包继续执行 500,000 bytes 上限。
- [合规] V130 只提供公开资讯、数据和关联依据，不生成目标价、收益预测、买卖建议或交易指令。
- [测试] V130 覆盖三地代码归一化、加密符号隔离、四级稳定排序、行业推导、游客边界、中英文切换和独立发布门禁。

- [透明度] V129 免费“今日市场事件”支持按需展开来源详情，展示最多 8 条去重后的发布方、来源代码、事件时间和安全原文链接；来源总数仍最多统计 20 条。
- [安全] V129 后端与前端双层限制原文链接只允许 HTTP/HTTPS；旧缓存中的异常协议链接会降级为不可点击，不影响事件浏览。
- [兼容] V129 事件 API 新增 `source_records`，旧缓存缺少该字段时前端会从事件主来源合成一条兼容记录。
- [合规] V129 来源对照只表示公开记录聚合，不代表事实已被独立证实；不新增网络请求、AI 调用、额度消耗、目标价、收益预测或交易指令。
- [测试] V129 新增来源明细去重、8 条上限、危险链接清洗、旧缓存兼容、中英文展开/折叠和独立发布门禁测试。

- [留存] V128 为免费“今日市场事件”增加浏览器本地已读基线；首次访问不把已有事件误标为新增，后续回访只提示新出现的事件，并支持全部标为已读。
- [透明度] V128 合并同标题公开事件时保留去重后的来源记录数量和发布方名称；多条来源记录只表示收录情况，不代表事实已被独立证实。
- [隐私] V128 本地状态最多保存 200 个事件 ID，不保存邮箱、自选、查询内容、标题、原文链接或 API Key，也不上传到服务端。
- [合规] V128 不新增网络请求、AI 调用或额度消耗，不修改原始资讯、历史报告、数据库或用户数据；所有内容只提供资讯和数据，不构成投资建议。

- [准确性] V127 对免费“今日市场事件”增加确定性相关度评分，过滤没有证券关联或财经市场信号的一般社会快讯，并对跨市场重复标题只保留一条。
- [留存] V127 新增全部市场、A股、港股、美股筛选，显示重点事件数量、重要度与可读的保留原因；登录用户的自选相关事件优先展示。
- [兼容] V127 事件 API 新增 `relevance_score`、`importance` 和 `relevance_reasons`，前后端均为旧缓存响应提供安全默认值。
- [合规] V127 仅提纯事件中心展示，不删除原始资讯，不新增抓取或 AI 调用，不生成目标价、收益预测或交易指令。
- [测试] V127 新增噪声过滤、跨市场去重、旧缓存兼容、市场筛选、自选优先、双语解释和独立发布门禁测试。

- [新功能] V126 首页新增免费“今日市场事件”，将现有 A股、港股、美股公开资讯按财报、公告、分红回购、交易状态、宏观和公司事项归类，并支持直接进入关联股票查询。
- [留存] V126 在浏览器端突出自选股相关事件，游客和免费用户看到相同公开事件结构；自选信息不写入公共首页 API 或公共缓存。
- [性能] V126 只复用已加载的公开首页资讯，不新增网络请求；事件中心使用懒加载，继续执行 `HomePage` 主包小于 500,000 bytes 门禁。
- [合规] V126 不调用 AI、平台 API、BYOK、Ollama 或 Kronos，不生成目标价、收益预测或交易指令，并持续展示来源、时效和“不构成投资建议”提示。
- [测试] V126 新增事件分类、去重、证券关联、故障降级、双语筛选、私有自选接线和独立发布门禁测试。
- [准确性] V126 对标题中明确出现的 `.SH`、`.SZ`、`.HK` 交易所代码优先按真实市场归类，避免混合资讯源把 A 股公告误标为港股；“净利润”类标题统一归入财报业绩；央行逆回购和工厂订单优先归入宏观数据。
- [准确性] V126 自动关联证券时使用代码边界，1 至 2 位美股代码只接受 `$MU` 这类明确 cashtag，避免 `A`、`AI` 等普通文字误关联；未关联证券的市场标签明确显示为“资讯源”。

- [新功能] V124 首页新增免费“今日市场工作台”，集中展示 A股、港股、美股市场状态、当地时间、主要指数和跨市场资讯时间线。
- [修复] V124 跨市场时间线优先保留每个可用市场的最新一条资讯，再按时间补齐，避免单一市场挤占港股或美股内容。
- [留存] V124 使用浏览器本地最近查看记录提供“继续研究”入口，同一股票去重置顶并最多保留六条，不写账户或 API Key 数据。
- [合规] V124 完全复用现有公开市场首页数据，不新增第三方抓取，不调用 AI、BYOK、Ollama 或 Kronos，并继续明确“仅提供资讯和数据，不构成投资建议”。
- [测试] V124 新增最近查看模型、双语工作台、首页点击留存和独立发布门禁测试。

- [新功能] V122 客服站内通知在侧边栏展示用户未读客服回复数和管理员待处理工单数，打开工单或更新状态后立即刷新。
- [性能] V122 摘要接口不返回主题或消息正文；页面仅在可见时每 60 秒轮询，会话或工单变更时立即刷新。
- [测试] V122 新增用户隔离、管理员权限、摘要脱敏、页面可见性、`99+` 边界、桌面和移动端角标回归与独立验收器。

- [新功能] V121 动态首页榜单新增按需公开数据预览，游客点击后可查看开高低昨收、成交量额、MA5/MA10/MA20、5日/20日变化、量能对比、行业、市值和近期收盘曲线。
- [性能] V121 首屏不预取股票详情；点击后立即展示榜单现价、涨跌、成交额和来源，同时按需补充均线、曲线与公司资料；市场或榜单切换会取消旧结果，失败时榜单保持可用并支持重试。
- [合规] V121 数据预览不要求登录、不调用 AI、不消耗平台 API、BYOK、Ollama 或 Kronos 额度，只提供客观数据位置描述并明确不构成投资建议。
- [测试] 新增 V121 组件、首页集成和独立门禁，覆盖中英文、缺失字段降级、按需请求、错误重试、完整查询入口和移动端边界。

- [新功能] V119 首页将固定关注股票替换为 A 股、港股、美股全市场活跃榜、涨幅榜和跌幅榜；A 股新增公开行业热点，股票可直接进入免费查询。
- [性能] V119 三市场榜单并发拉取并使用 120 秒热缓存和最近成功缓存降级；公开网络实测冷加载约 1.8 秒、热缓存毫秒级。
- [合规] V119 首页榜单只使用公开市场数据，不调用 AI、不消耗 API 额度；数据源失败时不以固定股票冒充热门，并继续标注来源、新鲜度和“不构成投资建议”。
- [测试] 新增 V119 动态市场首页门禁，覆盖三市场映射、行业热点、缓存降级、无固定池、前端榜单切换和构建。

- [体验] V118 首页接入上证指数、恒生指数、标普500指数及财联社、格隆汇、金十数据公开白名单，使用短超时、限大小、去重与缓存；来源失败时诚实降级，且不会自动调用平台 API、BYOK、Ollama 或 Kronos。

- [体验] V118 将首页重构为三地市场、主要指数、关注行情和来源可追溯资讯优先的财经信息面，隐藏旧版空白分析占位。
- [体验] 已登录账户区收纳为摘要、个人工作台和账户入口；模型选择移入账户页，首页不自动运行 API 或本地模型。
- [测试] 新增 V118 首页信息架构门禁，覆盖公开资讯、无 API 自动运行、账户点击进入、中英文和合规边界。

- [验收] 新增 V117 六项功能与 Ollama 本地真实验收门禁，覆盖 Kronos、A 股增强、AlphaSift、Financial Services、市场工作台和市场脉搏。
- [修复] 港股/美股详情补充公开行情与历史数据兜底，修复直达股票时旧市场总览覆盖新结果的请求竞态。
- [体验] 市场来源、时间、数值与降级原因按中英文显示；游客不再建立需登录的任务事件流，筛选结果可在浏览器会话内恢复。
...
- [新功能] 新增 V112 本地 API 加油包、会员模型目录、三步 BYOK 接入和用户自有 Ollama 轻量连接器；只提供资讯和数据，不提供投资建议。
- [安全] V112 对用户密钥按用户与记录 ID 二次校验，白名单为空时关闭 BYOK，连接器仅访问回环 Ollama 且云端只保存设备令牌哈希和加密任务。
- [测试] 新增 V112 加油包幂等、额度原子性、模型权限、跨用户连接器隔离和 Windows/macOS 发布门禁测试。
- [验收] Windows 本地连接器已完成真实 Ollama 发现、统一模型目录、AAPL 资讯分析、额度扣减、EXE 构建/启动和本地下载验证；macOS 真机、正式签名与公证仍未验证。
...
- [新功能] 新增 V101 免费历史走势研究台：游客快速分析可查看 30/90/180/365 日真实收盘价、MA5/MA20、成交量、区间表现、最大回撤和后续观察条件，全程未使用 AI。
- [性能] 新增 V102 A 股免费查询加速：公告、资金流、板块、研报和龙虎榜改为 1.5 秒总预算内并发抓取，超时通道独立降级并短期缓存，重复查询可直接复用进程内结果。
- [新功能] 新增 V103 免费用户每日研究驾驶舱：把自选股分为重点确认、风险复核和等待确认，展示证据、下一观察条件、失效条件、数据可信度与免费平台 API 试用边界，全程默认未使用 AI。
...
- [新功能] 新增 V120 平台客服中心：注册用户可提交、回复和关闭文本工单，管理员可在运营后台处理队列；工单按用户隔离，复用 CSRF、限流和审计，默认不启用 AI 自动回复或附件。
