...
- 游客首次进入首页时，公开行情、主要指数、三地关注股票、公开资讯标题、来源和更新时间优先于注册、额度和模型设置展示。
- 首页可以自动加载不消耗模型额度的公开互联网资讯和行情；任何平台 API、BYOK、Ollama 或 Kronos 分析都必须由用户主动查询后运行。
- 公开资讯必须保留发布方、时间和原始链接；没有正式资讯时只展示明确标注为“基于行情”的客观变化，不得伪造新闻或事件。
- 已登录首页只显示账户摘要、套餐、每周额度、个人工作台入口和账户入口；API Key、模型选择和完整账户详情放在 `/account`。
- 自选、今日复盘和条件提醒保留在按需展开的个人工作台内，不得因首页收纳而删除用户能力或数据。
- 主页不得自动执行 AI 以制造内容密度，也不得展示买卖建议、目标价、仓位或收益承诺。

## V119 动态市场首页

- 首页热门股票必须来自全市场公开排序结果，不得继续使用固定股票池并包装为“热门”或“全市场排名”。
- A 股和港股使用公开市场成交额、涨幅、跌幅排序；美股使用公开 screener，并根据可用字段标记盘前、盘中、盘后或休市阶段。
- A 股行业热点使用公开行业板块数据；港股和美股没有可靠行业源时必须明确显示暂不可用，不得编造行业热度。
- 榜单允许过滤退市、新股异常波动或低流动性样本，以提高免费首页可读性；筛选依据必须在页面说明。
- 所有公开请求必须设置硬超时、并发上限、短期缓存和最近成功缓存降级；单市场失败不得清空其他市场。
- 首页榜单、指数和资讯不得触发平台 API、BYOK、Ollama、Kronos 或任何 AI 配额；点击股票后才进入用户主动查询流程。
- 首页必须展示来源、新鲜度、缓存状态和“仅提供资讯和数据，不构成投资建议”；不得输出买卖指令、目标价、仓位或收益预测。

## V120 平台客服中心

- 客服工单必须绑定 `platform_user_id`；普通用户只能读取、回复和关闭自己的工单，跨用户访问统一返回不存在。
- 管理员队列必须要求平台管理员或既有本地管理员会话；普通平台用户不得读取全量工单。
- 所有客服写操作必须通过 CSRF 校验和独立限流桶，关闭工单后不得继续回复，除非管理员显式重新打开。
- 用户消息触发管理员未读，管理员消息或状态变化触发用户未读；读取详情只清除当前一侧未读。
- 审计只能记录工单 ID、分类和状态，不得记录消息正文、密码、API Key、Cookie 或其他秘密。
- V120 只支持文本人工客服，不启用 AI 自动回复、附件、图片、外部客服渠道或 LukaAI 数据库共享。
- 客服仅处理产品使用、账户、行情来源、模型连接和故障问题，不得提供买卖指令、目标价、仓位、收益承诺或个性化投资建议。

## V121 免费市场股票数据预览

- 游客和免费用户可从动态首页榜单按需查看单只股票公开数据详情，不要求登录，也不消耗平台 API、BYOK、Ollama 或 Kronos 额度。
- 首页首屏不得预取单股详情；只有用户明确点击“数据详情”后才调用既有公开市场工作台接口，每次只展开一个股票。
- 详情补充请求较慢时必须先展示榜单已有的现价、涨跌、成交额和来源，不得让用户只等待空白转圈。
- 数据详情包括开高低昨收、成交量额、MA5/MA10/MA20、5日与20日变化、量能对比、行业、市值、近期收盘曲线、来源状态和更新时间。
- 缺失字段必须显示“暂不可用”，不得用零、模型推测或固定样例填充；请求失败必须保留榜单并允许重试。
- 市场或榜单切换必须关闭旧详情并使较旧请求失效，避免跨市场结果覆盖当前选择。
- 客观数据位置只描述最新价相对均线和成交量相对均量的位置，不得输出未来方向、买卖指令、目标价、仓位或收益预测。
- 中文和英文模式必须完整切换；桌面与移动端均不得出现横向溢出。
- V121 只代表本地公开数据功能，不批准生产部署、真实支付、生产密钥或市场数据商业再分发。

## V122 客服站内通知

- 普通用户摘要必须按当前平台用户隔离；管理员摘要必须复用现有管理员权限。
- 摘要仅允许数量和最早等待时间，不得返回主题、消息正文、邮箱或密钥。
- 轮询仅在页面可见时执行，周期不得小于 60 秒；会话和工单变化可立即刷新。
- V122 不启用 AI 自动回复、WebSocket、邮件、短信、飞书、真实支付、生产部署或投资建议。

## V124 免费每日市场工作台

- 游客首页可查看 A股、港股、美股市场状态、当地时间、主要指数、跨市场资讯时间线和浏览器本地最近查看，不要求登录。
- 工作台只复用 V119/V121 已有公开市场响应，不新增第三方数据源，也不调用平台 API、BYOK、Ollama、Kronos 或其他 AI 配额。
- 跨市场时间线优先使用资讯发布时间；缺少发布时间时必须明确标为观察时间或抓取时间，不得伪装为实时事件。
- 最近查看仅保存在当前浏览器，字段限定为股票代码、名称、市场和查看时间，最多六条；不得写入邮箱、用户 ID、API Key、Cookie 或账户数据。
- 点击榜单、指数、行业领涨标的、数据预览或最近查看时，可进入现有免费查询；不得自动发起 AI 分析。
- 免费版与高级版使用同一信息架构；高级版差异只体现在允许的数据源、刷新频率和 API 查询额度，不隐藏公开事实数据。
- 页面必须同时提供中文和英文，并保留“仅提供资讯和数据，不构成投资建议”；不得输出买卖指令、目标价、仓位或收益预测。

## V138 A股与港股历史事件覆盖

- 历史事件只接受公开来源明确记录的计划日期或已发生公司事件日期；新闻发布时间、抓取时间和模型推测不得替代事件日期。
- A 股实际披露来自巨潮资讯，港美股历史公司行动来自 Yahoo 公共图表；这些公开通道不等于已取得交易所实时或商业再分发授权。
- 存在指定证券集合时，证券事件必须先按精确代码过滤；无代码宏观事件可保留，不得让全市场其他公司挤掉指定标的。
- 最多六条观察必须为有候选的 A股、港股、美股保留市场席位，再按事件日期填充，单市场失败不得阻断其他市场。
- 页面和 API 必须区分来源计划日期与来源历史事件，并保留来源、新鲜度、缓存和不可用状态。
- 历史行情和同期基准只用于客观对照；不得宣称事件导致价格变化，不得生成投资建议、买卖指令、目标价、仓位或收益预测。
- 生产模板默认关闭 V138；本地开启时仍不得调用 AI、读取用户 API Key、写账户数据库、发送通知或修改历史报告。

## V141 同类事件历史对比

- V141 复用 V139/V140 的单证券事件档案响应，在同一接口中返回 `comparison_summaries`；不得增加第二次前端请求、重复事件源或新的配额桶。
- 统计范围只包含当前证券最多 24 条真实公开事件，按事件类型及 1/3/5/20 个交易日观察窗口聚合；不得混入其他证券、行业均值或模型生成样本。
- 每个窗口可展示个股、市场基准和相对变化的有效样本数、中位数及区间，并展示正、负、持平样本数和完整度。缺失、布尔值和非有限值必须排除，不得用零补齐。
- 少于 3 个有效个股样本时必须显示“样本有限”；没有有效观察时必须显示不可用状态。不得用有限样本生成置信度、胜率、评级或推荐排序。
- V141 的事件类型、观察窗口和历史样本选择必须与 V139 列表和 V140 K 线联动，避免同一页面展示互相矛盾的事件状态。
- 中文与英文界面必须完整覆盖标题、统计字段、样本状态和免责声明；外部证券名称与数据源专名可以保留来源原文。
- 历史分布不预测未来表现，同期变化不证明事件与价格之间存在因果关系。V141 只提供资讯和历史数据，不构成投资建议。
- V141 继承 V139 的游客只读、独立匿名限流和生产默认关闭边界；不得读取身份、用户 API Key、账户数据库或支付状态，不得调用平台模型、BYOK、Ollama 或 Kronos。

## V140 事件与K线联动图

- V140 复用 V139 的单证券事件档案响应，在同一接口中返回 `chart`；不得新增重复的前端行情请求、第二套事件源或另一个独立配额桶。
- 图表仅支持已纳入市场工作台的 A 股、港股和美股证券，分别使用上证指数、恒生指数和标普500指数作为市场基准。
- 图表时间范围跟随 6/12/24 个月档案选择，最多返回 560 个日线点；各序列以所选区间首个有效收盘为零点。市场基准只按完全相同的交易日期配对，不前填、后填或伪造缺失值。
- 事件类型筛选和 1/3/5/20 个交易日观察窗口必须同时驱动图表与事件列表；基准缺失时保留个股线，行情缺失时保留事件档案并明确说明降级。
- 中文与英文界面必须完整覆盖图表标题、状态、来源、事件选择和非因果声明；外部证券名称与数据源专名可以保留来源原文。
- V140 是公开历史数据可视化，不是 Kronos 预测、AI 结论、事件影响评级或收益预测。不得输出买卖建议、目标价、仓位、交易指令或收益承诺。
- V140 继承 V139 的游客只读、独立匿名限流和生产默认关闭边界；不得读取身份、用户 API Key、历史报告、账户数据库或支付状态，不得调用平台模型、BYOK、Ollama 或 Kronos。

## V139 个股事件档案

- 游客和免费用户可在单只 A 股、港股或美股的市场工作区查看 6/12/24 个月公开事件档案，不要求登录、API Key 或 AI。
- 档案必须精确匹配当前证券，不得混入同市场其他公司；最多返回 24 条，网络读取受超时、响应体和历史记录数量限制。
- 当前真实事件来源只覆盖巨潮定期报告实际披露、Yahoo 公开财报日历、分红除息和拆股。股份回购与重要公告只有在接入可追溯来源后才能显示，禁止用新闻标题或模型推测补造。
- 每个事件保留事件日期、发布方、来源链接、来源状态和抓取时间；行情观察保留独立的行情来源、新鲜度和缺失状态。
- 1/3/5/20 个交易日变化只表示同一时间窗口内的客观行情数据，不证明事件导致上涨或下跌，不得生成影响评级、买卖指令、目标价、仓位或收益预测。
- V139 API 公开只读并使用独立匿名 IP 限流；不得解析用户身份、读取用户 API Key、调用 AI、写账户数据库、发送通知或修改历史报告。
- `.env.example` 和生产模板默认关闭 V139；本地开启不代表取得市场数据商业再分发授权或批准公网发布。

## V142 个股研究总览与同业对比

- 快速查询必须先展示一个统一研究总览，包含研究总览、财务趋势、同业对比、资讯事件、K线样本、来源与边界六个标签页；旧版详细模块默认收起但不得删除。
- 游客和免费用户可读取 A股、港股、美股公开研究数据，不要求登录、API Key 或 AI；加密货币财务报表必须明确标为不适用。
- 年度财务最多保留五个财年，只展示公开源实际返回的收入、净利润、摊薄每股收益、经营现金流和增长率；缺失、非有限或不可用字段不得补零或推测。
- 历史市盈率位置必须明确说明计算方法为财年末观察价格除以当年摊薄每股收益；不得包装为实时估值、预测估值、目标估值或合理价格。
- 财务接口按需加载并缓存，失败时只降级财务标签，不得阻断行情、同业、事件、K线和来源标签。
- 同业对比必须展示来源、新鲜度和缺失状态；不得将固定同行样本描述为完整行业排名。
- 中文和英文界面必须完整覆盖标签、加载、空状态、来源和边界；证券名称、行业名和数据源专名可保留来源原文。
- V142 只提供资讯和数据，不构成投资建议或交易指令；不得生成目标价、收益预测、仓位、止损或买卖建议，不得调用平台模型、BYOK、Ollama、Kronos 或公共搜索。
