...
## 2026-07-30 V141 同类事件历史对比

- V139 个股事件档案响应已扩展 `comparison_summaries`，基于同一批最多 24 条事件及已有 1/3/5/20 日观察生成描述统计，不增加外部数据请求。
- 页面新增中英文同类事件样本数、中位数、区间、正负分布、完整度和有限样本提示；历史样本按钮与 V140 K 线当前事件联动。
- 缺失、布尔值和非有限数值不计入统计，也不补零；不足 3 个有效样本或暂无历史观察均以真实降级状态显示。
- V141 不调用 AI、BYOK、Ollama、Kronos 或用户密钥；历史分布不预测未来表现、不证明因果关系，也不构成投资建议。
- 独立门禁 `scripts/verify_platform_symbol_event_comparison_v141.py` 已通过并输出 `DSA_PLATFORM_SYMBOL_EVENT_COMPARISON_V141_OK`；V136-V140、发布候选包、Local V1 operability 和 V2 readiness 门禁均通过。
- 后端跨版本回归 120 项通过；前端聚焦回归 4 文件 / 21 项通过，全量 Vitest 为 139 文件 / 1144 项通过 / 2 项按配置跳过，ESLint 与生产构建通过。
- 8018 实际接口缓存验收：AAPL 返回 4 个事件、501 个图表点和 2 类对比，腾讯控股返回 3 个事件、491 个图表点和 2 类对比，贵州茅台返回 1 个事件、485 个图表点和 1 类对比；三者均为 `ai_used=false`、`informational_only=true`、`causality_disclaimer=true`。
- 真实浏览器验收覆盖 AAPL、`0700.HK` 和 `600519.SH`；AAPL 在 V141 选择 4 月 30 日财报样本后，V140 同步选中并显示个股、基准及相对变化。中英文无平台文案混杂，桌面与 `390x844` 手机视口无横向溢出，控制台 error 为 0。

## 2026-07-30 V140 事件与K线联动图

- V139 个股事件档案响应已扩展 `chart`，复用现有公开历史行情缓存和超时边界，不新增平行接口或第二次前端请求。
- A 股、港股和美股分别使用上证指数、恒生指数和标普500指数作基准；6/12/24 个月最多返回 560 个日线点，基准只在同一日期存在时配对。
- 页面新增中英文标准化走势、事件标线、事件选择与 1/3/5/20 个交易日观察窗口联动；基准或行情缺失均按真实状态降级。
- V140 不调用 AI、BYOK、Ollama、Kronos 或用户密钥；图表仅展示公开历史同期数据，不表示事件导致价格变化，也不构成投资建议。
- 独立门禁 `scripts/verify_platform_event_kline_timeline_v140.py` 已通过并输出 `DSA_PLATFORM_EVENT_KLINE_TIMELINE_V140_OK`；V140-V136、发布候选包、Local V1 operability 和 V2 readiness 门禁均通过。
- 后端相关回归 134 项通过；前端聚焦回归 3 文件 / 16 项通过，全量 Vitest 退出码为 0，ESLint 与生产构建通过。
- 8018 实际接口验收：AAPL 返回 4 个事件和 126 个图表点，`0700.HK` 返回 3 个事件和 122 个图表点，`600519.SH` 返回 1 个事件和 121 个图表点；三者均为 `ai_used=false`、`informational_only=true`。
- 真实浏览器验收覆盖 AAPL、腾讯控股和贵州茅台，6/12/24 个月、事件选择及 1/3/5/20 日窗口均可联动；中英文基准名称完整，桌面和 `390x844` 手机视口无横向溢出，浏览器控制台 error 为 0。

## 2026-07-30 V139 个股事件档案

- 个股市场工作区新增公开事件档案，游客和免费用户无需登录、API Key 或 AI，可选择 6/12/24 个月、事件类型及 1/3/5/20 个交易日观察窗口。
- 后端按证券精确过滤并复用 V138 的巨潮资讯、Yahoo 公开日历、历史公司行动和事件后行情观察链；档案接口与首页六条摘要互不覆盖。
- 24 个月档案将公开公司行动和历史行情回看扩展为两年有界读取，最多保留 24 个事件和 560 条日线记录。
- 页面显示事件来源、来源链接、新鲜度、事件后个股变化、市场基准变化和相对变化；缺失数据保持不可用，不补造数值。
- 当前实际来源覆盖定期披露、分红除息和拆股；股份回购、重要公告筛选只有取得可追溯来源后才显示真实项目。
- 档案请求失败不阻塞基础行情；中文和英文使用同一结构，并持续显示“同期变化不代表事件导致价格变化”和“不构成投资建议”。
- 8018 真实浏览器验收覆盖 `300750`、`HK09988` 和 `NVDA`，并确认工作区返回的裸 A 股代码和 `HK` 前缀港股代码会分别规范化为 `300750.SZ`、`9988.HK`。
- A 股、港股、美股分别展示 2、3、4 条可追溯事件；24 个月、3 个交易日、来源链接、中英文切换均可用。
- `1440x1000` 桌面和 `390x844` 手机视口均无横向溢出；最终控制台 error 为 0。
- 前端全量回归为 137 files / 1135 passed / 2 skipped；V139 聚焦门禁为 38 个后端测试和 2 files / 12 个前端测试。
- `scripts/verify_platform_symbol_event_archive_v139.py` 通过时输出 `DSA_PLATFORM_SYMBOL_EVENT_ARCHIVE_V139_OK`。
- V139 生产模板默认关闭，不调用 AI、不读取用户 API Key、不写数据库、不发送通知；本地验收不代表市场数据商业再分发或公网发布获批。

## 2026-07-30 V138 A股与港股历史事件覆盖

- 巨潮实际披露记录使用 `provider_event_history / observed`，预约日期继续使用 `provider_schedule / scheduled`。
- 指定证券集合按精确代码优先过滤，避免全市场其他公司事件在有界截断前挤掉关注标的。
- 港股和美股新增 Yahoo 公共图表历史公司行动通道，可读取历史除息和拆股日期；历史财报通道失败时彼此独立。
- V138 首页摘要仍默认回看 180 天；底层上限由 V139 扩展为最多 760 天，行情读取范围扩展到两年、最多 560 个交易日记录。
- 最多六条结果为有候选的 A股、港股、美股保留市场席位，再按日期填充。
- API 与中英文页面显示来源计划日期或来源历史事件，继续显示来源、新鲜度、缓存和三市场可用性。
- 真实公开源探针取得 A 股 4 条、港股 8 条、美股 19 条候选；观察结果同时包含 A股、港股和美股。
- 8018 真实浏览器验收取得 A股 5 条、港股 8 条、美股 17 条可用事件；中文和英文均显示计划日期/历史事件依据、宁德时代实际披露和阿里巴巴除息观察。1日/3日窗口切换会更新观察交易日和同期数据，`9988.HK` 查询可进入港股工作区。
- 1440 桌面和 `390x844` 移动视口均无横向溢出、空白页或控制台错误；页面持续显示来源、新鲜度、未使用 AI、同期表现不代表因果关系及“不构成投资建议”边界。
- `scripts/verify_platform_event_history_coverage_v138.py` 通过时输出 `DSA_PLATFORM_EVENT_HISTORY_COVERAGE_V138_OK`。
- V138 不调用 AI、不读取 API Key、不写用户数据库、不发送通知；同期表现不解释为事件导致行情变化，只提供资讯和数据，不构成投资建议。

## 2026-07-30 V137 事件数据可用性与冷启动提速

- V136 事件后市场观察新增版本化公开磁盘快照；服务重启后可优先返回最近一次通过边界校验的公开事件和历史行情观察，再在后台刷新。
- 快照只允许事件观察响应字段，强制 `ai_used=false` 和 `informational_only=true`，递归拒绝 API Key、令牌、Cookie、用户 ID、邮箱等敏感字段；文件最大 1 MiB，使用同目录临时文件和原子替换。
- A股、港股、美股事件候选改为三个独立任务；单一市场超时只把该市场标为不可用，其他市场结果继续显示。三市场全部不可用时不会覆盖上一份有效磁盘快照。
- 公开事件观察不再同步构建完整市场首页；日历超时任务按查询键复用，避免重复请求持续堆积慢任务。
- 本地开关启用时，FastAPI 启动阶段只安排一次无用户、无密钥的单飞预热；生产模板和通用示例仍默认关闭，不配置 Windows 自动启动。
- 前端显示 A股、港股、美股来源状态、内存/磁盘快照来源和后台刷新状态；没有快照时先显示正在刷新，而不是让整页等待慢源。后台最多轮询 6 次，成功市场数据不会被失败市场清空。
- 所有来源状态、缓存状态和降级提示支持中文与英文；若响应出现 `ai_used=true` 或 `informational_only=false`，前端继续失败关闭。
- 磁盘快照、启动预热和三市场独立刷新均不调用 AI、不读取平台或用户 API Key、不写用户数据库、不发送通知。
- 页面继续明确说明事件后同期表现仅是历史数据观察，不证明因果关系；只提供资讯和数据，不生成目标价、收益预测、仓位、买卖或交易指令。
- 独立门禁成功标记为 `DSA_PLATFORM_EVENT_DATA_AVAILABILITY_V137_OK`。
- 本地 8018 首次启动完成后台预热后生成 4.8 KB、版本 1 的公开快照；实测响应含 3 条观察，来源状态为 A股新鲜 0 条、港股新鲜 0 条、美股新鲜 5 条，不把没有事件的市场伪装成有数据。
- 再次重启后，`/api/v1/market-workspace/event-reactions` 在 3 ms 内从磁盘返回 3 条观察，`ai_used=false`、`informational_only=true`；随后后台刷新可替换为进程内新鲜缓存。
- 真实浏览器验收覆盖中文、英文、1/3/5/20 日切换、MSFT 查询跳转、1440 桌面和 390 移动宽度；无横向溢出、无空白页、无浏览器控制台错误。

## 2026-07-30 V136 事件后市场观察

- 免费首页在 V135 真实市场日历之后新增“事件后市场观察”，游客和登录用户使用同一个公开只读端点，不要求 API Key 或 AI。
- 只处理已经发生且来源明确标记为 `time_kind=scheduled`、`classification_source=provider_schedule` 的最近 6 个事件，不把普通新闻发布时间当成事件基准。
- 事件候选从最近 45 日真实日历中按新到旧选择；历史读取会补齐跨窗口所需的上一巨潮报告期和上一年度 FOMC 日程。观察窗口以事件所在市场的事件日收盘为基线，按市场基准交易日确定后续 1/3/5/20 日，展示证券同期变化、市场基准同期变化、相对基准变化和观察窗口量能比。
- 个股停牌或缺少目标市场交易日行情时保持不可计算，不会跳过停牌日后拿更晚交易日冒充目标窗口；周末和节假日是否成熟按实际市场交易日判断。
- 事件所在市场的当日尚未结束时不提前计算；A股、港股和美股分别使用上海、香港和纽约时区判断事件日期。
- A股、港股、美股基准分别使用上证指数、恒生指数和标普500指数；FOMC 计划事件使用标普500指数作为市场观察对象，不映射到单只证券。
- Yahoo Chart 仅作为 HTTPS 无密钥公开历史行情通道，不声称交易所实时数据、官方授权数据或完整企业行动调整数据；页面显示来源和新鲜度。
- 行情代码去重并行读取，总等待上限 4 秒；响应缓存 900 秒，刷新失败时最多使用 21600 秒陈旧缓存并保留原抓取时间；历史行情缓存最多保留 32 个代码，公共日历缓存最多保留 64 个键，每个来源缓存最多保留 72 条事件。
- 数据不足显示“等待足够交易日”或“历史行情不足”，不补造收益；事件源全部超时或部分成功但没有任何可用事件时不会缓存成正常空结果。部分来源失败时前端保留成功事件并显示降级告警；若后端响应出现 `ai_used=true` 或 `informational_only=false`，前端拒绝展示整批数据。
- 公开端点使用 `market_workspace_event_reactions` 独立 IP 限流桶，即使全局平台限流关闭仍强制执行，进程内限流状态最多保留 4096 个身份桶；容量已满时拒绝新身份而不淘汰活跃桶。端点不解析用户身份，前端不上传用户标识或跨站凭据，同源浏览器即使附带已有会话 Cookie 也不参与功能判断；不读取平台或用户 API Key、不调用 AI、不写数据库、不发送通知。
- 页面明确说明同期表现不代表事件导致行情变化；只提供资讯和数据，不生成因果结论、影响评级、目标价、收益预测、仓位、买卖或交易指令。
- `.env.example` 和生产模板默认关闭，仅本地忽略的 `.env` 开启；独立门禁成功标记为 `DSA_PLATFORM_OBSERVED_EVENT_REACTIONS_V136_OK`。

## 2026-07-30 V135 真实市场日历与本周市场大事

- 平台认证启用且游客未登录时，首页不再请求历史、自选、设置状态和分析技能等私有资源；公开市场首页、真实市场日历和免登录查询继续加载。本地旧模式在平台认证关闭时保持原有工作台能力。
- 免费首页新增真实公开市场日历，默认显示未来 7 日“本周市场大事”，并支持“今日事件 / 本周大事 / 我的日历 / 待复盘”四个视图。
- A股计划财报日期读取巨潮资讯定期报告预约；港美股财报和除息日读取 Yahoo Finance 的 yfinance 无密钥公开通道；美股宏观计划读取美联储 FOMC 官方页面。
- 巨潮和美联储属于官方公开源；Yahoo/yfinance 仅按无密钥公开数据通道展示，不声称获得官方授权或实时行情许可。
- 真实计划事件标记 `time_kind=scheduled` 和 `classification_source=provider_schedule`；只读取日期，不显示盈利、营收、目标价或收益预测。
- 三类来源并行执行，整体等待上限 4 秒；来源分别缓存 900 秒，刷新失败时最多使用 21600 秒陈旧缓存并明确标记。
- V135 最多返回 36 条真实日程；和 V126 新闻事件合并、去重后首页最多 48 条。单来源失败保持其他市场和既有新闻可用。
- 游客无需登录、API Key 或 AI；页面显示来源、新鲜度和查询入口，复用 V133/V134 浏览器本地关注及复盘状态，不写数据库。
- 事件与行情并列不代表因果关系；本功能只提供公开资讯和数据，不生成投资建议、目标价、收益预测、仓位、买卖或交易指令。
- `.env.example` 和生产模板默认关闭；仅本地 `.env` 开启进行 8018 浏览器验收。
- 独立门禁为 `scripts/verify_platform_real_market_calendar_v135.py`，成功标记为 `DSA_PLATFORM_REAL_MARKET_CALENDAR_V135_OK`。

## 2026-07-30 V134 免费市场日历与本地提醒

- 免费“今日市场事件”新增“免费市场日历 / Free market calendar”，统一整理 A 股、港股、美股公开事件和 V133 关注后的 1/3/5/20 日复盘节点。
- 只有公开来源标题或摘要明确给出日期时才显示“计划日期”；其余事件显示“发布时间”，不会把新闻发布时刻伪装为未来日程。
- 日历显示市场、日期依据、来源、来源状态和关联证券，并提供“全部日历 / 我的日历 / 待复盘”筛选与免费查询入口。
- 当日公开事件较多时，默认列表优先保留最多 4 个近期关注复盘节点，避免关注操作完成后节点不可见。
- 日历仅保留最近 7 日和未来 30 日、最多 48 条；待复盘已查看状态最多 128 条，保存在浏览器本地。
- 游客登录后只把当前游客查看状态迁入 `user-<数字ID>`，不同用户互不共享，不保存邮箱，不上传提醒状态。
- 本阶段复用首页已加载公开事件和 V133 关注状态，不新增网络请求、AI、API 额度、数据库表、邮件、短信或推送。
- 事件与行情并列不代表因果关系；日历只整理公开资讯和本地检查点，不生成目标价、收益预测、买卖建议、仓位或交易指令。
- 独立门禁为 `scripts/verify_platform_free_market_calendar_v134.py`，成功标记为 `DSA_PLATFORM_FREE_MARKET_CALENDAR_V134_OK`。

## 2026-07-29 V133 市场事件后续追踪

- 游客可在 V132 事件研究卡中关注具有关联证券的公开事件，关注后首页出现“事件后续追踪 / Event follow-up”。
- 追踪记录展示事件事实、关注时公开行情基准、最新观察、相对变化和 1/3/5/20 日观察节点；没有观察时明确显示待观察或暂无记录。
- 同一证券晚于原事件的公开事件会作为后续来源更新展示；证券代码采用精确规范化匹配，不做包含式匹配。
- 数据只写浏览器本地，最多 12 个关注事件、每项 32 条观察；游客登录后迁入当前 `user-<数字ID>`，不同用户互不共享，不保存邮箱。
- 本阶段复用首页已加载公开事件和行情，不新增网络请求、AI、API 额度、数据库表或服务端用户追踪。
- 事件与后续行情并列展示不代表因果关系；仅提供公开资讯和数据，不生成目标价、收益预测、买卖建议、仓位或交易指令。
- 独立门禁为 `scripts/verify_platform_market_event_follow_up_v133.py`，成功标记为 `DSA_PLATFORM_MARKET_EVENT_FOLLOW_UP_V133_OK`。

## 2026-07-15 V132 事件驱动研究闭环

- 公开事件有关联证券时显示“研究事件 / Research event”，用户可在首页展开独立研究卡，不要求登录。
- 研究卡展示事件标题、来源数量、事件时间、关联证券，以及首页已经加载的最新价、涨跌幅、成交量、成交额、行情来源、新鲜度和观测时间。
- A股、港股和美股候选行情从全部市场的关注列表、活跃榜、涨幅榜和跌幅榜去重汇总，不受当前市场页签限制；指数不参与关联证券匹配，避免指数代码与个股代码碰撞。
- 公开榜单不含关联证券时明确说明数据缺失，并保留进入现有免费个股查询的入口，不伪造行情或趋势。
- 价格变化与事件同时呈现不代表因果关系；页面只提供公开资讯和数据，不生成目标价、收益预测、买卖建议、仓位或交易指令。
- 本阶段不新增网络请求、AI、API 额度、数据库或服务端用户追踪；独立门禁为 `scripts/verify_platform_event_driven_research_v132.py`。

## 2026-07-15 V131 回访用户事件收件箱

- 游客和登录用户再次访问“今日市场事件”时，可以显式切换到“只看新增 / New only”，快速找到浏览器本地基线之后出现的公开事件。
- 摘要展示总事件、新增、重点，以及有自选时的自选相关数量；市场和类别筛选继续约束当前列表，但不改写完整事件计数。
- “只看新增”本身不会把事件标记为已读；只有打开事件、查询关联证券或点击“全部标为已读”等明确操作才更新本地已读基线。
- 读完全部新增事件后显示真实空状态，并提供返回全部事件入口；完整公开事件流不会被删除或隐藏。
- 本阶段不上传已读状态、自选、筛选偏好或用户标识，不新增网络请求、AI、API 额度、数据库或服务端追踪。
- 所有内容仅提供公开资讯和数据，不构成投资建议；独立门禁为 `scripts/verify_platform_returning_event_inbox_v131.py`，成功标记为 `DSA_PLATFORM_RETURNING_EVENT_INBOX_V131_OK`。
- 本轮全量回归同时修复三个既有图标按钮的原生 `title` 违规，并让本地连接器配对码测试等待倒计时 effect 生效；不改变数据、支付、账户或模型路由。

## 2026-07-15 V130 自选关联市场事件

- 登录且已有自选的用户默认看到“为我优先”，事件按自选股、相关行业、关注市场、普通事件四级排序；“全部事件”可恢复相关度/时间排序。
- 个性化只调整当前公共事件流的顺序，不隐藏事件、不修改来源、不改写标题，也不把固定股票包装成市场热点。
- 行业关联只复用已经加载的公共首页证券资料；自选代码、行业推导和排序都在浏览器端完成，公共首页接口不接收用户标识或自选数据。
- A股、港股、美股代码统一归一化；`BTC-USD` 等加密符号不会被误归为美股关注市场。
- 游客和无自选用户继续看到普通公共事件流，不显示个性化按钮或匹配标签。
- 中英文均覆盖“为我优先 / 全部事件”和三类匹配说明，并持续展示“只提供公开资讯和数据，不构成投资建议”。
- 独立门禁：`scripts/verify_platform_personalized_market_events_v130.py`，成功标记为 `DSA_PLATFORM_PERSONALIZED_MARKET_EVENTS_V130_OK`。

## 2026-07-15 V129 事件来源对照与证据详情

- 免费“今日市场事件”可按需展开最多 8 条去重来源明细，显示发布方、来源代码、事件时间和可用原文链接；默认折叠，不挤占首屏。
- 来源总数最多统计 20 条，明细最多返回 8 条；完全重复来源不重复计数。
- 原文链接在服务层和浏览器映射层均只接受 HTTP/HTTPS，异常协议降级为“原文链接不可用”。
- 旧缓存没有 `source_records` 时，前端从事件主来源合成最多一条兼容记录，页面不会因版本漂移崩溃。
- 多条来源记录不代表事实已独立证实；本阶段不新增抓取、AI、API 额度消耗或服务端用户追踪，只提供公开资讯和数据，不构成投资建议。
- 独立门禁：`scripts/verify_platform_event_source_evidence_v129.py`，成功标记为 `DSA_PLATFORM_EVENT_SOURCE_EVIDENCE_V129_OK`。
...
## V126 免费每日市场事件状态（2026-07-14）

- 首页将已经加载的 A股、港股、美股公开资讯归一化为每日市场事件，不增加第三方抓取或额外网络请求。
- 事件使用确定性关键词规则分为财报、公告、分红回购、交易状态、宏观、公司事项和一般市场动态；分类来源明确标为 `keyword_rules`。
- 标题含明确 `.SH`、`.SZ`、`.HK` 交易所代码时，事件市场以代码为准，并将“净利润”变化归入财报业绩，避免混合资讯频道造成错误市场标签。
- 自动关联证券使用代码边界；1 至 2 位美股代码必须有 `$` cashtag，避免普通文字误关联。未关联证券的市场标签显示为资讯源，央行逆回购和工厂订单按宏观数据处理。
- 只有标题明确包含榜单股票代码或名称时才关联股票，点击后进入现有免费查询；不会根据正文猜测标的。
- 登录用户的自选代码只在浏览器内用于显示“自选关注”，不写入公共首页响应、缓存或事件源。
- 事件服务失败时返回空事件并标记 `market_events_unavailable`，三市场行情、榜单和资讯继续可用。
- 中文和英文界面覆盖筛选、来源状态、时间语义、空状态及资讯数据边界；事件中心使用懒加载。
- `scripts/verify_platform_free_daily_market_events_v126.py` 通过时输出 `DSA_PLATFORM_FREE_DAILY_MARKET_EVENTS_V126_OK`。
- 本状态仅代表本地免费公开数据验收，不代表生产部署、真实支付、生产密钥、市场数据商业授权或法律审批；所有内容只提供资讯和数据，不构成投资建议。

## V127 免费市场事件提纯与相关性状态（2026-07-15）

- 免费事件中心只保留结构化事件、明确关联证券或包含财经市场信号的公开资讯；低相关社会和外交快讯仍保留在原始资讯区，不进入重点事件列表。
- 同一标题在 A股、港股和美股资讯通道重复出现时全局去重，避免有限首页空间被重复快讯占用。
- 每条保留事件返回 `0..100` 的相关度、`high / medium / low` 重要度和可解释原因；这些字段由确定性规则产生，不是 AI 判断或投资评级。
- 中文和英文页面支持全部市场、A股、港股、美股筛选，显示重点事件数量、重要度、保留原因和来源状态。
- 登录用户的自选相关事件只在浏览器端优先排序，不写入公共 API、缓存或资讯源；游客与免费用户仍可查看相同公开事件。
- 旧缓存事件缺少 V127 字段时按 `0 / low / []` 安全降级，不影响首页加载。
- V127 不新增第三方抓取、不调用平台 API、BYOK、Ollama、Kronos 或其他 AI 模型，不修改历史报告、数据库或用户数据。
- `scripts/verify_platform_market_event_relevance_v127.py` 通过时输出 `DSA_PLATFORM_MARKET_EVENT_RELEVANCE_V127_OK`。
- 本状态仅代表本地免费公开数据验收，不代表生产部署、支付、生产密钥、市场数据商业授权或法律审批；所有内容只提供资讯和数据，不构成投资建议。

## V128 事件信源透明与回访留存状态（2026-07-15）

- 同一规范化标题由多个不同公开来源记录收录时仍只展示一条事件，同时返回最多 20 条去重来源记录计数和最多 8 个发布方名称。
- 来源数量用于说明公开收录范围；多条来源记录不代表事实已被独立证实，也不构成来源评级或真实性保证。
- 首次访问只在浏览器建立当前事件基线，不把已有事件误标为新增；后续回访仅对未见过的稳定事件 ID 显示“新事件”，并支持全部标为已读。
- 本地状态最多保存 200 个事件 ID，不保存邮箱、自选、查询内容、标题、原文链接或 API Key；存储不可用或损坏时首页继续正常展示。
- V128 不新增第三方抓取、网络请求、AI 调用或额度消耗，不上传已读状态，不删除原始资讯，不修改历史报告、数据库或用户数据。
- 中文和英文界面均展示来源记录、发布方、新事件状态、全部标为已读及资讯数据边界。
- `scripts/verify_platform_event_source_retention_v128.py` 通过时输出 `DSA_PLATFORM_EVENT_SOURCE_RETENTION_V128_OK`。
- 本状态仅代表本地免费公开数据验收，不代表生产部署、支付、生产密钥、市场数据商业授权或法律审批；所有内容只提供资讯和数据，不构成投资建议。

## V142 个股研究总览本地状态（2026-07-30）

- 快速查询新增六标签页个股研究总览，游客和免费用户可查看行情、财务趋势、同业、资讯事件、K线样本及来源边界。
- A股、港股、美股年度财务通过独立按需接口获取，最多五个财年；加密货币明确标记财务报表不适用。
- 历史市盈率样本公开计算方法、观察数量、区间和百分位，不冒充实时、预测或目标估值。
- 旧版长页面详细模块默认收起并可一键展开，历史操作入口和现有功能未删除。
- 财务数据使用六小时缓存；来源失败、字段缺失和数据不足均明确降级，不补造数值，也不阻断基础查询。
- `scripts/verify_platform_stock_research_overview_v142.py` 通过时输出 `DSA_PLATFORM_STOCK_RESEARCH_OVERVIEW_V142_OK`。
- V142 不调用 AI、BYOK、Ollama、Kronos 或公共搜索，不读取用户 API Key，不修改历史报告、数据库或账户数据。
- 本状态只代表本地功能验收，不代表生产部署、真实支付、生产密钥、市场数据商业授权或法律审批；所有内容只提供资讯和数据，不构成投资建议。
