...
- [x] 展示关联证券最新价、涨跌幅、成交量和成交额；空值统一显示“暂不可用 / Unavailable”，并覆盖最新价部分缺失的中英文回归。
...
- [x] verifier 实际执行 V132 聚焦 Vitest，并检查首页包 500,000 bytes 上限。
...
- 新增指数与个股同号碰撞回归：`000001.SZ` 事件不得复用 `000001.SH` 上证指数行情；测试先失败并暴露错误指数价格，排除指数候选后通过。
- 全量前端为 130 个文件、1,088 项通过、2 项按既有配置跳过；lint 与生产构建通过。
- 后端事件服务、V126-V132 verifier 与发布包共 125 项通过；七个阶段 verifier 均输出正式 OK 标记。
- 最终首页包 `HomePage-ZHbIL_5F.js` 为 482,836 bytes，低于 500,000 bytes 门限。
- 8018 中文、英文、桌面和 390×844 移动视口复验无横向溢出或控制台错误；缺失行情降级、关闭和关联证券查询路径通过。
